<div class="rent-car-slide">
    <a href="{{ LaravelLocalization::getLocalizedUrl(null, route('website.yachts.show', ['yacht' => $yacht])) }}">
        <div class="car-rent-title">
            <picture>
                <img src="{{asset("/storage/") . "/" . \App\Helpers\WebpImage::generateUrl($yacht->image)}}" alt="">
            ...
            <div class="rent-car-slide-description-blue-boxes">
                @if ($yacht->price_per_day)
                    <div class="rent-car-slide-description-blue-box">
                        <span>{{__('lang.Day')}} / {{app('currencies')->convert($yacht->price_per_day)}} {{app('currencies')->getCurrency()->code}}</span>
                    </div>
                @endif
                @if($yacht->price_per_week)
                    <div class="rent-car-slide-description-blue-box">
                        <span>{{__('lang.Week')}}/{{app('currencies')->convert($yacht->price_per_week)}} {{app('currencies')->getCurrency()->code}}</span>
                    </div>
                @endif
                @if($yacht->price_per_month)
                    <div class="rent-car-slide-description-blue-box">
                        <span>{{__('lang.Month')}} / {{app('currencies')->convert($yacht->price_per_month)}} {{app('currencies')->getCurrency()->code}}</span>
                    </div>
                @endif
