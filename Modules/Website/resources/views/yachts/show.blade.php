...
@section('seo')
    @include('website::layouts.parts.seo', [
        'seo' => \App\Models\SEO::where('type', 'car')->where('resource_id', $yacht->id)->first(),
        "title" => $yacht->name,
...
@section("content")
    <main id="car-details">
            ...
            <div class="container m-auto ">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="car-details-slider">
                            <div class="slider-for-car-details">
                                @foreach($yacht->images as $image)
                                    <img class="mw-100 " src="/storage/{{\App\Helpers\WebpImage::generateUrl($image->image_url)}}" alt="{{$yacht->name}}">
                                @endforeach
                            </div>
                        ...
                        @if (app('settings')->get($yacht->type . "_notes"))
                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="car-details-require car-details-pricing">
                            ...
                            <ul class="mt-3">
                                @if ($yacht->company->payment_methods)
                                    <li>
                                        {{__('lang.Payment Methods')}} / {{collect(explode(',', $yacht->company->payment_methods))->map(function ($item){return __('admin.' . $item);})->implode(', ')}}
                                    </li>
                                @endif
                                <li>
                                ...
                                @if ($yacht->company->hours()->where('type', '!=', 'holiday')->count())
                                    <li>
                                        <div class="times-group-container d-flex gap-2">
                                            ...
                                            <div class="days">
                                                @foreach($yacht->company->hours()->where('type', '!=', 'holiday')->get() as $day)
                                                    <p> {{$day->day}} </p>
                                                @endforeach
                                            </div>
                                            <div class="hours">
                                                @foreach($yacht->company->hours()->where('type', '!=', '')->get() as $day)
                                                    @if ($day->type == "24")
        ...
        @if ($yacht->features->count() > 0)
            <section class="car-features">
                <div class="container">
                    ...
                    <div class="row">
                        @foreach ($yacht->features?->split(4) as $chunk)
        ...
        @endif
        @if ($content = \App\Models\Content::where('type', 'car')->where('resource_id', $yacht->id)->first())
